// Check for secure context and handle appropriately
const isSecureContext = window.isSecureContext || location.protocol === 'https:' || location.hostname === 'localhost';

// Create styles without inline style attribute
const createStyles = () => {
...
// Initialize IndexedDB with secure context check
const initDB = () => {
    return new Promise((resolve, reject) => {
        if (!isSecureContext) {
            console.warn('Not in a secure context. Using memory storage fallback.');
            resolve({
                _storage: new Map(),
                transaction: false,
...
// Media devices with secure context check
...
async function startVideo() {
    try {
        stream = await getMediaDevices().getUserMedia({ video: true });
...
const initializeApp = async () => {
    try {
        ...
        // Initialize camera if in secure context
        if (isSecureContext) {
            await startVideo().catch(error => {
                console.warn('Camera initialization deferred:', error);
            });
        } else {
            console.warn('Camera features disabled: Requires secure context (HTTPS or localhost)');
        }
