...
class UIManager {
  ...
  async createImageCard(image) {
    return await SecurityManager.ensureSecureOperation(async () => {
      ...
      try {
        // Use low-resolution DataURL for thumbnail in non-secure context
      ...
      expandIcon.addEventListener('click', () => {
        this.showFullscreen(image.data).catch(console.error);
      });
      ...
      const card = document.createElement('div');
      card.className = 'bg-white rounded-lg shadow-lg p-4';
      card.textContent = 'Image preview not available in non-secure context';
      return card;
    });
  }
...
class SecurityManager {
  ...
  static async ensureSecureOperation(operation, fallback = null) {
    try {
      SecurityManager.checkSecureContext();
      return await operation();
    } catch (error) {
      console.warn('Secure operation failed:', error);
...
class CameraApp {
  ...
  async init() {
    return await SecurityManager.ensureSecureOperation(
      async () => {
        // Full functionality in secure context
        if (await this.dbManager.init()) {
          this.uiManager = new UIManager(this.dbManager, this.cameraManager);
          await this.uiManager.displayImages();
          return true;
        }
        return false;
      },
      async () => {
        // Limited functionality in non-secure context
        console.warn('Running in fallback mode with limited functionality');
        this.uiManager = new UIManager(null, this.cameraManager);
        return true;
      }
    );
  }
