...
// Start video with iOS 14 and modern browser compatibility
async function startVideo() {
    try {
        // Define multiple constraint sets for broader compatibility
        const constraintSets = [
            // Most compatible, basic constraints
            { 
                video: { 
        ...
        // Attempt to get camera stream with multiple constraint sets
        let stream = null;
        for (const constraints of constraintSets) {
            try {
                ...
                console.warn('Camera constraints failed:', constraintError);
                continue;
            }
        }

        // If no stream is found, fall back to default
        if (!stream) {
        ...
        return new Promise((resolve, reject) => {
            // iOS 14 requires a more forgiving metadata loading
            ...
            const initializeVideoCapture = async () => {
                try {
                    // Slight delay to ensure video is fully initialized
        ...
        document.body.innerHTML = `
            <h1 class="text-red-500">
                Camera Access Error: ${error.message || 'Unknown error'}
                <br>
