...
// Polyfill for getUserMedia across different browser versions
const getMediaDevices = () => {
    // Check standard navigator.mediaDevices
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        return navigator.mediaDevices;
    }
    
    // Fallback to older browser prefixed versions
    const browserGetUserMedia = 
        navigator.getUserMedia || 
        navigator.webkitGetUserMedia || 
        navigator.mozGetUserMedia || 
        navigator.msGetUserMedia;
    
    if (browserGetUserMedia) {
        return {
            getUserMedia: (constraints) => {
                return new Promise((resolve, reject) => {
                    browserGetUserMedia.call(navigator, constraints, resolve, reject);
                });
            }
        };
    }
    
    // If no getUserMedia method is found
...
// Toggle camera with enhanced error handling and compatibility
...
// Capture image with fallback and error handling
const captureImage = () => {
    return new Promise((resolve, reject) => {
        try {
            requestAnimationFrame(() => {
                try {
                    // Fallback dimensions if video metadata is not available
